...
class GameNoTweens extends FlxState
{
	// contants
	...
	override public function create()
	{
		...
		headText.text = "Game prototype without tweens\n(As far as I can tell, the bareboned mechanics code is foolproofed)";
		add(headText);
	...
	function clickBtn():Void
	{
		...
		FlxG.switchState(new GameWithTweens());
	}
	...
	function swapIcons(row1:Int, col1:Int, row2:Int, col2:Int)
	{
		...
		if (col1 > col2)
		{
			icons[p1].x -= TILE_SIZE;
			icons[p2].x += TILE_SIZE;
		}
		if (col1 < col2)
		{
			icons[p1].x += TILE_SIZE;
		...
		if (row1 > row2)
		{
			icons[p1].y -= TILE_SIZE;
			icons[p2].y += TILE_SIZE;
		}
		if (row1 < row2)
		{
			icons[p1].y += TILE_SIZE;
		...
		{
			onSwapCompleted(row1, col1, row2, col2);
		}, 200);
	}

	/**
	 * If chain occurs will be removed else will swap back
	 * @param row1 
	 * @param col1 
	 * @param row2 
	 * @param col2 
	 */
	function onSwapCompleted(row1:Int, col1:Int, row2:Int, col2:Int):Void
	{
